                                ...
                                <tr class="{{ $loop->even ? 'bg-gray-50' : '' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <form action="{{ route('cart.update', $item['product']) }}" method="POST" class="inline-block">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <button type="submit" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline text-xs">Mettre à jour</button>
                                            </div>
                                        </form>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <form action="{{ route('cart.remove', $item['product']) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline text-xs" onclick="return confirm('Êtes-vous sûr de retirer ce produit du panier ?')">Retirer</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
                        ...
                        <tfoot class="bg-gray-200">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-3 text-right">
                                    <a href="{{ route('products.indexClient') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mr-2">Continuer vos achats</a>
                                    <a href="{{ route('orders.checkout') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Passer à la caisse</a>
                                </td>
                            </tr>
                        </tfoot>
                    </table>
