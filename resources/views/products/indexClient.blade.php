...
<div class="bg-gray-50 pt-15">
    <div class=" w-[50%]  mx-auto">
        <form action="{{ route('products.search') }}" method="GET" class="flex items-center">
            <label for="search" class="sr-only">Rechercher des produits</label>
            <input type="text" id="search" name="query"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                placeholder="Rechercher des produits..."
                value="{{ request('query') }}"
            >
            <button type="submit" class="bg-green-600 hover:bg-white hover:text-green-600 hover:border hover:border-green-500 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline ml-2"
            >
                Rechercher
            </button>
        </form>
    ...
    <div class="container w-[75%]  mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            @forelse ($products as $product)
                <div class="bg-white shadow-lg border border-gray-100 animate-fadeInUp animate-delay-500 rounded-xl overflow-hidden hover:scale-105 transition-transform">
                    <div class="relative">
                        @if ($product->image)
                        ...
                        <div class="absolute top-2 right-2">
                            <span class="inline-block rounded-lg px-1 pb-1 text-xs font-bold {{ $product->is_available ? 'bg-green-500 text-white' : 'bg-red-500 text-white' }}">
